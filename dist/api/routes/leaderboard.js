...
leaderboardRouter.get('/', (req, res) => {
  ...
  `).all(parseInt(limit, 10));
    ...
    `).all(bot.id);
        ...
        return Object.assign(Object.assign({}, bot), { unrealized_pnl: unrealizedPnl, total_pnl: bot.realized_pnl + unrealizedPnl });
    });
    // Re-sort by total P&L
    enriched.sort((a, b) => b.total_pnl - a.total_pnl);
    res.json({
        leaderboard: enriched.map((bot, i) => (Object.assign({ rank: i + 1 }, bot))),
    });
});
