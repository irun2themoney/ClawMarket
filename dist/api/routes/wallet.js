...
walletRouter.post('/register', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (agentId) {
            const existing = db.prepare('SELECT id, wallet_address, balance_usdc FROM bots WHERE openclaw_agent_id = ?').get(agentId);
            if (existing) {
                res.json({
                    botId: existing.id,
        ...
        const existingName = db.prepare('SELECT id, wallet_address, balance_usdc FROM bots WHERE name = ?').get(name);
        if (existingName) {
            res.json({
    ...
    `).run(botId, name, agentId || null, address.toLowerCase(), encryptedKey, initialBalance, now, now);
        console.log(`[wallet] registered bot "${name}" (${botId}) with wallet ${address}, balance: $${(initialBalance / 1000000).toFixed(2)}`);
        res.json({
            botId,
            walletAddress: address,
            balance: initialBalance,
            message: `Bot registered with $${(initialBalance / 1000000).toFixed(2)} USDC starting balance.`,
            existing: false,
        });
    }
    catch (err) {
...
walletRouter.post('/:id/deposit', (req, res) => {
    ...
    const db = getDb();
    const bot = db.prepare('SELECT id, name, balance_usdc FROM bots WHERE id = ?').get(botId);
    if (!bot) {
    ...
    // Record transaction
    db.prepare('INSERT INTO transactions (id, bot_id, type, amount, chain, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(uuid(), botId, 'deposit', amountMicro, 'internal', 'confirmed', now);
    const newBalance = bot.balance_usdc + amountMicro;
    console.log(`[wallet] deposited $${amount.toFixed(2)} to bot "${bot.name}" (${botId}), new balance: $${(newBalance / 1000000).toFixed(2)}`);
    res.json({
        botId,
        deposited: amountMicro,
        depositedUsdc: amount,
        newBalance,
        newBalanceUsdc: newBalance / 1000000,
    });
});
// GET /api/bots/:id/key — get encrypted key for SDK (requires bot auth)
walletRouter.get('/:id/key', (req, res) => {
    const botId = req.params.id;
    const authBotId = req.headers['x-bot-id'];
    ...
    const db = getDb();
    const bot = db.prepare('SELECT wallet_encrypted_key FROM bots WHERE id = ?').get(botId);
    if (!bot) {
        res.status(404).json({ error: 'Bot not found' });
        return;
    }
...
walletRouter.get('/:botId/address', (req, res) => {
    const db = getDb();
    const bot = db.prepare('SELECT wallet_address FROM bots WHERE id = ?').get(req.params.botId);
    if (!bot) {
        res.status(404).json({ error: 'Bot not found' });
        return;
    }
...
walletRouter.get('/:botId/onchain', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const db = getDb();
    const bot = db.prepare('SELECT wallet_address FROM bots WHERE id = ?').get(req.params.botId);
    if (!bot) {
        res.status(404).json({ error: 'Bot not found' });
    ...
    try {
        const balance = yield getUsdcBalance(bot.wallet_address);
        res.json({
            walletAddress: bot.wallet_address,
            onChainBalance: balance.toString(),
            onChainBalanceUsdc: (Number(balance) / 1000000).toFixed(6),
        });
    }
    catch (err) {
