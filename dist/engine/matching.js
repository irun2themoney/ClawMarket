...
export class MatchingEngine extends EventEmitter {
    ...
    submitOrder(input) {
        ...
        const market = db.prepare('SELECT status FROM markets WHERE id = ?').get(input.marketId);
    ...
    matchOrder(orderId, input) {
        ...
        while (remaining > 0) {
            ...
            if (input.orderType === 'limit') {
                if (input.side === 'buy' && counter.price > input.price)
                    break;
                if (input.side === 'sell' && counter.price < input.price)
                    break;
            }
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(tradeId, input.marketId, makerOrderId, takerOrderId, makerBotId, takerBotId, input.outcome, fillPrice, fillSize, feeAmount, feeRate, now);
            // Update maker order
    ...
    updatePosition(botId, marketId, outcome, sizeDelta, price) {
        const db = getDb();
    ...
    cancelOrder(orderId, botId) {
        const db = getDb();
        const order = db.prepare('SELECT * FROM orders WHERE id = ? AND bot_id = ?').get(orderId, botId);
