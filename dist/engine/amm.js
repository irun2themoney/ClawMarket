 ...
 */
export function getAmmPrice(marketId, outcome, side) {
    const db = getDb();
 ...
 */
export function ammBuy(botId, marketId, outcome, size, maxPrice, takerOrderId) {
    const db = getDb();
    ...
    const buyer = db.prepare('SELECT balance_usdc FROM bots WHERE id = ?').get(botId);
    ...
    VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(tradeId, marketId, takerOrderId || null, HOUSE_BOT_ID, botId, outcome, ammPrice, size, feeAmount, 0.02, now);
    // Record fee in treasury
    const currentTreasury = db.prepare('SELECT COALESCE(MAX(balance_after), 0) as bal FROM treasury').get();
    const newBalance = ((currentTreasury === null || currentTreasury === void 0 ? void 0 : currentTreasury.bal) || 0) + feeAmount;
 ...
 * @param takerOrderId - The taker's order ID (for trade records)
 */
export function ammSell(botId, marketId, outcome, size, minPrice, takerOrderId) {
    const db = getDb();
    ...
    const position = db.prepare('SELECT shares FROM positions WHERE bot_id = ? AND market_id = ? AND outcome = ?').get(botId, marketId, outcome);
    ...
    INSERT INTO trades (id, market_id, maker_order_id, taker_order_id, maker_bot_id, taker_bot_id,
      outcome, price, size, fee_amount, fee_rate, created_at)
    VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(tradeId, marketId, takerOrderId || null, botId, HOUSE_BOT_ID, outcome, ammPrice, size, feeAmount, 0.02, now);
    // Record fee in treasury
    const currentTreasury = db.prepare('SELECT COALESCE(MAX(balance_after), 0) as bal FROM treasury').get();
    const newBalance = ((currentTreasury === null || currentTreasury === void 0 ? void 0 : currentTreasury.bal) || 0) + feeAmount;
    db.prepare('INSERT INTO treasury (id, trade_id, amount, balance_after, created_at) VALUES (?, ?, ?, ?, ?)')
...
function updatePosition(db, botId, marketId, outcome, sizeDelta, price) {
    const existing = db.prepare('SELECT shares, avg_price FROM positions WHERE bot_id = ? AND market_id = ? AND outcome = ?').get(botId, marketId, outcome);
    if (!existing) {
        if (sizeDelta > 0) {
