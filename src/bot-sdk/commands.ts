 ...
 */
export async function handleCommand(bot: ClawMarketBot, input: string): Promise<string> {
  const parts = input.trim().split(/\s+/);
  if (parts[0]?.toLowerCase() !== 'clawmarket') {
    return '';
  }
  ...
  try {
    switch (cmd) {
      case 'status':
        return await cmdStatus(bot);
      case 'markets':
        return await cmdMarkets(bot, parts.slice(2));
      case 'buy':
        return await cmdBuy(bot, parts.slice(2));
      case 'sell':
        return await cmdSell(bot, parts.slice(2));
      case 'portfolio':
        return await cmdPortfolio(bot);
      case 'wallet':
        return cmdWallet(bot);
      case 'leaderboard':
        return await cmdLeaderboard(bot);
      case 'auto':
...
async function cmdStatus(bot: ClawMarketBot): Promise<string> {
  const { balance, positions } = await bot.getBalance();
  const pnl = await bot.getPnL();

  let msg = `**ClawMarket Status**\n`;
  msg += `Balance: $${(balance / 1_000_000).toFixed(2)} USDC\n`;
  msg += `P&L: $${(pnl.total / 1_000_000).toFixed(2)} (realized: $${(pnl.realized / 1_000_000).toFixed(2)}, unrealized: $${(pnl.unrealized / 1_000_000).toFixed(2)})\n`;
  msg += `Active positions: ${positions.length}\n`;
  msg += `Auto-trading: ${bot.isAutoTrading() ? 'ON' : 'OFF'}`;
  return msg;
}

async function cmdMarkets(bot: ClawMarketBot, args: string[]): Promise<string> {
  const category = args[0] || undefined;
  const markets = await bot.getMarkets({ category });
...
async function cmdBuy(bot: ClawMarketBot, args: string[]): Promise<string> {
  ...
  const result = await bot.buy(marketId, outcome as 'yes' | 'no', amount, price);

  return `Order placed: ${result.fillCount} fills, ${result.totalFilled} shares filled, $${(result.totalFees / 1_000_000).toFixed(4)} fees`;
}

async function cmdSell(bot: ClawMarketBot, args: string[]): Promise<string> {
  ...
  if (isNaN(shares) || shares <= 0) return 'Invalid shares amount';

  const price = priceStr ? parseFloat(priceStr) : undefined;
  const result = await bot.sell(marketId, outcome as 'yes' | 'no', shares, price);

  return `Sell order placed: ${result.fillCount} fills, ${result.totalFilled} shares sold`;
}

async function cmdPortfolio(bot: ClawMarketBot): Promise<string> {
  const { balance, positions } = await bot.getBalance();
  const pnl = await bot.getPnL();

  let msg = `**Portfolio**\n`;
  msg += `Balance: $${(balance / 1_000_000).toFixed(2)}\n`;
  msg += `Total P&L: $${(pnl.total / 1_000_000).toFixed(2)}\n\n`;
  ...
  if (positions.length === 0) {
    ...
    for (const p of positions as any[]) {
      ...
      msg += `- ${p.market_title || p.market_id}: ${p.shares} ${p.outcome.toUpperCase()} shares @ ${p.avg_price.toFixed(3)} (value: $${(value / 1_000_000).toFixed(2)})\n`;
    }
  }
...
function cmdWallet(bot: ClawMarketBot): string {
  return `**Wallet**\nAddress: \`${bot.getWalletAddress()}\`\nChain: Base L2 (eip155:8453)\nFund with USDC to start trading.`;
}

async function cmdLeaderboard(bot: ClawMarketBot): Promise<string> {
  const leaders = await bot.getLeaderboard();

  if (leaders.length === 0) return 'No bots on the leaderboard yet.';

  let msg = `**Leaderboard**\n\n`;
  for (const l of leaders.slice(0, 10)) {
    const pnlStr = (l.total_pnl / 1_000_000).toFixed(2);
...
function cmdAuto(bot: ClawMarketBot, toggle: string): string {
  if (toggle === 'on') {
