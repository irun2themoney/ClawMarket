...
export interface TradeSignal {
  ...
  price?: number;        // limit price (optional — omit for market order)
...
export interface StrategyContext {
  balance: number;        // available USDC balance in microcents
  positions: Array<{
    marketId: string;
    outcome: string;
    shares: number;
 ...
 */
export class PolymarketArbStrategy implements TradingStrategy {
  name = 'polymarket-arb';

  // Minimum price divergence to trigger a trade (5%)
  private threshold = 0.05;
  // Max position size as fraction of balance
  private maxPositionFraction = 0.1;

  evaluate(ctx: StrategyContext): TradeSignal[] {
    const signals: TradeSignal[] = [];

    if (ctx.balance <= 0) return signals;

    for (const snapshot of ctx.markets) {
      const { market } = snapshot;
      if (!market.poly_price_yes || !market.poly_price_no) continue;
      ...
      if (Math.abs(yesDivergence) > this.threshold) {
        if (yesDivergence > 0) {
          ...
          const yesPosition = ctx.positions.find(
            p => p.marketId === market.id && p.outcome === 'yes' && p.shares > 0
          );
          if (yesPosition) {
            signals.push({
              action: 'sell',
