 ...
 */
export function settleMarket(marketId: string, resolution: 'yes' | 'no'): SettlementResult {
  const db = getDb();
  const now = Date.now();

  // Get all positions for this market
  const positions = db.prepare(
    'SELECT bot_id, outcome, shares, avg_price FROM positions WHERE market_id = ? AND shares > 0'
  ).all(marketId) as Array<{ bot_id: string; outcome: string; shares: number; avg_price: number }>;
  ...
  const updatePosition = db.prepare(
    'UPDATE positions SET shares = 0, realized_pnl = realized_pnl + ? WHERE bot_id = ? AND market_id = ? AND outcome = ?'
  );
