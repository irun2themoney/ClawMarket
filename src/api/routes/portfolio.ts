...
portfolioRouter.get('/:id/portfolio', (req: Request, res: Response) => {
  const db = getDb();
  ...
  const realizedPnl = db.prepare(
    'SELECT COALESCE(SUM(realized_pnl), 0) as total FROM positions WHERE bot_id = ?'
  ).get(botId) as { total: number };

  const tradeCount = db.prepare(
    'SELECT COUNT(*) as count FROM trades WHERE maker_bot_id = ? OR taker_bot_id = ?'
...
portfolioRouter.get('/:id/balance', (req: Request, res: Response) => {
  const db = getDb();
...
portfolioRouter.get('/:id/trades', (req: Request, res: Response) => {
  const db = getDb();
