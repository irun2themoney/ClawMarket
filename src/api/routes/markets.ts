...
marketsRouter.get('/:id/trades', (req: Request, res: Response) => {
  const db = getDb();
  const limit = (req.query.limit as string) || '50';
  const offset = (req.query.offset as string) || '0';
  ...
  const trades = db.prepare(`
    ...
    LIMIT ? OFFSET ?
  `).all(req.params.id, parseInt(limit as string, 10), parseInt(offset as string, 10));

  res.json({ trades });
});

// GET /api/markets/meta/categories — list unique categories
marketsRouter.get('/meta/categories', (_req: Request, res: Response) => {
  const db = getDb();
  const categories = db.prepare(
    "SELECT DISTINCT category FROM markets WHERE category IS NOT NULL AND status = 'active' ORDER BY category"
  ).all();
...
marketsRouter.patch('/:id', (req: Request, res: Response) => {
  const db = getDb();
