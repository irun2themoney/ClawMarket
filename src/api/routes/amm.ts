...
ammRouter.get('/price/:marketId', (req: Request, res: Response) => {
  const marketId = req.params.marketId;

  const db = getDb();
  const market = db.prepare('SELECT id, title, poly_price_yes, poly_price_no, price_yes, price_no FROM markets WHERE id = ?')
    .get(marketId) as { id: string; title: string; poly_price_yes: number | null; poly_price_no: number | null; price_yes: number | null; price_no: number | null } | undefined;
...
ammRouter.get('/house', (_req: Request, res: Response) => {
  const db = getDb();
  ...
  const totalPositions = db.prepare(
    'SELECT COUNT(*) as count FROM positions WHERE bot_id = ? AND shares > 0'
  ).get(HOUSE_BOT_ID) as { count: number };
